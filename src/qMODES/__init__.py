from .read_environment_variables import get_QMODES_ERA_DIR, get_QMODES_MODES_DIR, get_QMODES_COEF_DIR, get_QMODES_VSF_DIR, get_QMODES_VSFINT_DIR, get_QMODES_COEF_DIR, get_QMODES_QKDATA_DIR, get_QMODES_QMODESDATA_DIR, get_QMODES_PLOTS_DIR
...
##############################################################################
############################ OLD __init__.py BELOW ###########################
##############################################################################


####from .computations import compute_vsf_int, compute_qk, compute_qmodes
####from .data_readers import get_full_field_ERA_and_flipped_qmodes_data, get_full_field_ERA_and_flipped_qmodes_data_with_p_and_lat_dependent_background, get_single_plev_ERA_and_flipped_qmodes_data, get_single_plev_ERA_and_flipped_qmodes_data_with_p_and_lat_dependent_background
####from .math_util    import qMODES_deriv, qMODES_deriv_at_point
####from .parameters   import nK, nM, nN, nplev, nlat, nlon
####from .sample_files import sample_vsf_file, sample_vsf_int_file, sample_coef_file, sample_hough_file, sample_freq_file, sample_ERA_file
####from .templates    import template_vsf_fname, template_vsf_int_fname, template_hough_fname, template_coef_fname
####
####
#####--------------------- Checking if sample files exist ------------------------
####def _check_sample_file_functions():
####	
####	import os
####
####	sample_file_functions_list = [sample_vsf_file, sample_vsf_int_file, sample_coef_file, sample_hough_file, sample_freq_file, sample_ERA_file]
####	has_thrown_warning = False
####	
####	for sample_func in sample_file_functions_list:
####    
####    	#Print warning if file doesn't exist
####		if not os.path.isfile(sample_func()):
####
...
####			print(f"\t{sample_func.__name__}")
####
####	if has_thrown_warning: print("You should modify the paths in the module file sample_files.py\n")
####
####
####
#####----------------------- Print bkg function warning --------------------------
####def _print_bkg_function_warning():
####	"""
####	Function that warns the user about qmodes and qk functions not 
####	accounting for the background function in the computation, and 
